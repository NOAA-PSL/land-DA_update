...
class viirsFV3(object):
    ...
    def _read(self):
        ...
        sncv = ncd.variables['VIIRSscf'][:]
        sndv = ncd.variables['VIIRSsnd'][:]
        ...
        # YX: Daily VIIRS L3 file does not have a time stamp associated
        # YX: Per discussions with Clara on 07/25/2024, hard coded to append VIIRS obs. time as 12UTC
        # YX: For offline land DA workflow only, no tests have been done for coupled DA runs (July 2024)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=('Read viirsFV3 snow cover and depth file(s) and Converter'
                     ' of native NetCDF format for observations of snow'
                     ' cover and depth from viirsFV3 to IODA netCDF format.')
    )
    parser.add_argument('-i', '--input',
                        help="name of viirsFV3 snow input file(s)",
    ...
    # Read in the viirsFV3 snow data
    viirs = viirsFV3(args.input)

    # setup the IODA writer
    writer = iconv.IodaWriter(args.output, locationKeyList, DimDict)

    # write everything out
    writer.BuildIoda(viirs.outdata, VarDims, viirs.varAttrs, AttrData)
